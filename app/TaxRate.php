...
class TaxRate extends Model
{
    ...
    public static function forBusinessDropdown(
        ...
        //        return [$item->id => ['data-rate' => $item->amount, 'data-min_amount' => $item->min_amount,'data-sub_taxes' => $item->sub_taxes]];
        //    })->all();
        //  }
   ...
   if ($include_attributes) {
    $tax_attributes = collect($result)->mapWithKeys(function ($item) {
        ...
        if ($item->sub_taxes && $item->sub_taxes->count() > 0) {
            ...
            if ($firstSub) {
                $attr[$item->id]['data-rate']       = $firstSub->amount;
                $attr[$item->id]['data-min_amount'] = $firstSub->min_amount;
            }

            if ($secondSub) {
                $attr[$item->id]['data-tax2_rate']       = $secondSub->amount;
                $attr[$item->id]['data-tax2_min_amount'] = $secondSub->min_amount??0;
            }
        } else {
             $attr[$item->id]['data-rate']       = $item->amount;
            $attr[$item->id]['data-min_amount'] = $item->min_amount;
        }
