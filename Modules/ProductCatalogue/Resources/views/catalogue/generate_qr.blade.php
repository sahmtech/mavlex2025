        ...
        <div class="row">
            <div class="col-md-7">
                @component('components.widget', ['class' => 'box-solid'])
                    ...
                    <div class="form-group">
                        {!! Form::label('upload_file', __('productcatalogue::lang.upload_file') . ':') !!}
                        <input type="file" name="file" id="upload_file" class="form-control" accept=".pdf,.jpg,.jpeg,.png">
                        <input type="hidden" id="uploaded_file_url" value="">
                        <small class="text-muted">{{ __('productcatalogue::lang.upload_file_placeholder') }}</small>
                    </div>
                    <div class="form-group">
                        <div class="checkbox">
                            <label>
                    ...
                    <button type="button" class="tw-dw-btn tw-dw-btn-primary tw-dw-btn-sm tw-text-white"
                        id="generate_qr">@lang('productcatalogue::lang.generate_qr')</button>
                @endcomponent
                ...
                @component('components.widget', ['class' => 'box-solid'])
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            {!! Form::open([
                                'url' => action([
                                    \Modules\ProductCatalogue\Http\Controllers\ProductCatalogueController::class,
                                    'productCatalogueSetting',
                                ]),
                                'method' => 'post',
                            ]) !!}
                            ...
                            <div class="form-inline">
                                <div class="form-group">
                                    ...
                                    <div class="checkbox">
                                        <label>
                                            {!! Form::radio('is_show', 1, $is_show == 1 ? true : false, [
                                                'id' => 'show_logo',
                                                'class' => 'input-icheck',
                                                'required',
                                            ]) !!} @lang('productcatalogue::lang.show')
                                        </label>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <div class="checkbox">
                                        <label>
                                            {!! Form::radio('is_show', 0, $is_show == 0 ? true : false, [
                                                'id' => 'show_logo',
                                                'class' => 'input-icheck',
                                                'required',
                                            ]) !!} @lang('productcatalogue::lang.hide')
                                        </label>
                                    </div>
                                </div>
                            </div> <br>
                            <button type="submit" class="tw-dw-btn tw-dw-btn-primary tw-dw-btn-sm tw-text-white"
                                id="">@lang('productcatalogue::lang.save')</button>
                            {!! Form::close() !!}
                        </div>
                    </div>
                @endcomponent
            ...
            <div class="col-md-5">
                @component('components.widget', ['class' => 'box-solid'])
                    <div class="text-center">
                        ...
                        <br>
                        <a href="#" class="tw-dw-btn tw-dw-btn-success tw-text-white hide"
                            id="download_image">@lang('productcatalogue::lang.download_image')</a>
                    </div>
                @endcomponent
            </div>
        </div>
    ...
    <script type="text/javascript">
        (function($) {
            ...
            $(document).on('click', '#generate_qr', function(e) {
                var file_input = $('#upload_file').prop('files')[0];
                ...
                if (file_input) {
                    ...
                    $('#generate_qr').prop('disabled', true).text('Uploading...');

                    $.ajax({
                        url: "{{ route('product-catalogue.upload') }}",
                        type: 'POST',
                        data: formData,
                        processData: false,
                        contentType: false,
                        success: function(response) {
                            if (response.success) {
                                generateFinalQR(response.url);
                            }
                        },
                        error: function() {
                            alert("File upload failed. Generating default link instead.");
                            var defaultLink = "{{ url('catalogue/' . session('business.id')) }}/" +
                                location_id;
                            generateFinalQR(defaultLink);
                        },
                        complete: function() {
                            $('#generate_qr').prop('disabled', false).text("@lang('productcatalogue::lang.generate_qr')");
                        }
                    });
                }
                else {
                    var link = "{{ url('catalogue/' . session('business.id')) }}/" + location_id;
                    generateFinalQR(link);
                }
            });

            function generateFinalQR(link) {
                $('#qrcode').html('');
                ...
                if ($('#subtitle').val().trim() !== '') {
                    opts.subTitle = $('#subtitle').val(); // Fixed: was using #title
                ...
                // Ensure the canvas ID is set after the library renders it
                setTimeout(function() {
