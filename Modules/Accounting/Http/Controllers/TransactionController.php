        ...
        return DataTables::of($query)
            ...
            ->addColumn('action', function ($row) use ($transaction_type) {
                ...
                if (auth()->user()->can('accounting.transaction_create_Journal_entry')) {
                    ...
                    if (!$is_mapped) {
                        $html .= '<a  
                                href="' . route('create_Journal_entry', ['id' => $row->transaction_id])  . '" class="btn btn-primary btn-xs mt-10" ><i class="fas fa-link"></i> ' . __('accounting::lang.create_Journal_entry') . '</a>';
                    }
                    //  else {
                    //     $html .= '<a href="#" 
