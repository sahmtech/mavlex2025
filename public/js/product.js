...
$(document).ready(function () {
    ...
    $(document).on("change", "input#single_dpp", function (e) {
        ...
        var min_amount = $("select#tax").find(":selected").data("min_amount");
        tax_rate = tax_rate == undefined ? 0 : tax_rate;

        var tax_2_rate = $("select#tax").find(":selected").data("tax2_rate");
        var min_amount_2 = $("select#tax")
            .find(":selected")
            .data("tax2_min_amount");
        tax_2_rate = tax_2_rate == undefined ? 0 : tax_2_rate;

        var purchase_inc_tax = __add__percent(
            purchase_exc_tax,
            tax_rate,
            min_amount,
            tax_2_rate,
        ...
        var selling_price = __add__percent(purchase_exc_tax, profit_percent);
        __write_number($("input#single_dsp"), selling_price);

        var selling_price_inc_tax = __add__percent(
            selling_price,
            tax_rate,
            min_amount,
            tax_2_rate,
    ...
    $(document).on("change", "select#tax", function () {
        if ($("select#type").val() == "single") {
            var purchase_exc_tax = __read_number($("input#single_dpp"));
            purchase_exc_tax =
                purchase_exc_tax == undefined ? 0 : purchase_exc_tax;

            var tax_rate = $("select#tax").find(":selected").data("rate");
            var min_amount = $("select#tax")
                .find(":selected")
                .data("min_amount");
            tax_rate = tax_rate == undefined ? 0 : tax_rate;
    ...
    $(document).on("change", "input#single_dsp_inc_tax", function (e) {
        var tax_rate = $("select#tax").find(":selected").data("rate");
        tax_rate = tax_rate == undefined ? 0 : tax_rate;
        var selling_price_inc_tax = __read_number(
            $("input#single_dsp_inc_tax")
        );
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.success == true) {
                        ...
                        toastr.error(
                            __translate("skus_already_exists", {
                                sku: result.sku,
                            })
                        );
                        return false;
    ...
    $(document).on("change", "input.variable_dpp", function (e) {
        ...
        __write_number(
            tr_obj.find("input.variable_dpp_inc_tax"),
            purchase_inc_tax
        );

        var profit_percent = __read_number(
            tr_obj.find("input.variable_profit_percent")
        );
        var selling_price = __add__percent(purchase_exc_tax, profit_percent);
        ...
        __write_number(
            tr_obj.find("input.variable_dsp_inc_tax"),
            selling_price_inc_tax
        );
    });
                ...
                success: function (result) {
                    if (result) {
                        if (result.values.length > 0) {
                            ...
                            tr_obj
                                .find(".variation_template_values_div")
                                .removeClass("hide");
                            tr_obj
        ...
        var min_amount_2 = $("select#tax")
            ...
            $("table.variation_value_table > tbody").each(function () {
                $(this)
                    .find("tr")
                    .each(function () {
                        var purchase_exc_tax = __read_number(
                            $(this).find("input.variable_dpp")
                        );
                        purchase_exc_tax =
                            purchase_exc_tax == undefined
                                ? 0
                                : purchase_exc_tax;
                        ...
                        var selling_price = __read_number(
                            $(this).find("input.variable_dsp")
                        );
                        var selling_price_inc_tax = __add__percent(
                            selling_price,
                            tax_rate,
